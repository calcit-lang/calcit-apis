...
{} (:package |app)
  :configs $ {} (:init-fn |app.main/main!) (:reload-fn |app.main/reload!) (:modules $ [] |lilac/compact.cirru |memof/compact.cirru |respo.calcit/compact.cirru |respo-ui.calcit/compact.cirru |respo-markdown.calcit/compact.cirru |calcit-theme.calcit/compact.cirru |writer.calcit/compact.cirru |reel.calcit/compact.cirru) (:version nil)
  :files $ {}
    |app.comp.container $ {}
      :ns $ quote
      ...
      :defs $ {}
            ...
            list->
              {}
                :style $ merge ui/row-middle
                  {} (:cursor :pointer) (:font-family ui/font-fancy) (:color $ hsl 200 80 80) (:font-weight 300)
              ...
              list->
                ...
                ->> (:snippets info)
                  map-indexed $ fn (idx snippet)
                    [] idx $ let
                        code-snippet $ if (list? snippet) ({} $ :code snippet) (, snippet)
                        code $ :code code-snippet
                      div ({}) (comp-code code syntax)
                        if
                          and (map? code-snippet) (some? $ :result code-snippet)
                          div ({} $ :style ui/row)
                            div
                              {} $ :style
                ...
                :cirru-text $ pre
                  {}
                    :style $ {} (:font-family ui/font-code)
                      ...
                      :line-height "\"22px"
                      :margin "\"0px 0px"
                    :innerHTML $ trim (write-code $ [] code)
                  ...
                  filter $ fn (info)
                    and
                      ...
                      or (empty? $ :selected-tags state)
                        ->> (:selected-tags state)
                          every? $ fn (x)
                            contains? (:tags info) x
                      contains? (:name info) (:query state)
              div
                {} $ :style
                  merge ui/global ui/fullscreen ui/row $ {} (:background-color $ hsl 0 0 96)
                div
                  {} $ :style
              ...
              ->> ([] :list :map :number :string :set :syntax :macro :native)
                map $ fn (tag)
                  [] tag $ div
                    {}
                      :style $ merge
                        {} (:display :inline-block) (:background-color $ hsl 200 80 84) (:margin "\"4px 4px") (:padding "\"0 4px") (:color $ hsl 0 0 100) (:border-radius "\"4px") (:cursor :pointer) (:line-height "\"22px")
                        if
                          contains? (:selected-tags state) tag
        ...
        |comp-wip-switcher $ quote
          defcomp comp-wip-switcher (state cursor)
            div
              {}
                :style $ {} (:font-family ui/font-fancy) (:color $ hsl 200 80 70) (:font-weight 300) (:cursor :pointer)
